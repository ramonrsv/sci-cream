//! [`ChocolateSpec`] and associated implementations, for chocolate and other cocoa ingredients

use serde::{Deserialize, Serialize};

use crate::{
    composition::{
        Carbohydrates, Composition, Fats, Fibers, PAC, SimpleProteins, SimpleSolids, Solids, Sugars, ToComposition,
    },
    constants::{
        composition::cacao::{
            STD_ASH_IN_DUTCHED_COCOA_SOLIDS, STD_ASH_IN_NATURAL_COCOA_SOLIDS,
            STD_CARBOHYDRATES_IN_DUTCHED_COCOA_SOLIDS, STD_CARBOHYDRATES_IN_NATURAL_COCOA_SOLIDS,
            STD_COCOA_BUTTER_IN_CACAO_SOLIDS, STD_FIBER_IN_COCOA_SOLIDS, STD_PROTEIN_IN_COCOA_SOLIDS,
            STD_SATURATED_FAT_IN_COCOA_BUTTER, STD_WATER_IN_COCOA_POWDER,
        },
        hf,
    },
    error::Result,
    validate::{Validate, verify_are_positive, verify_is_100_percent, verify_is_subset},
};

#[cfg(doc)]
use crate::{
    composition::CompKey,
    constants::{self, composition::cacao},
};

/// Spec for chocolate ingredients, with cacao solids, cocoa butter, and optional sugar and others
///
/// See the [chocolate documentation](crate::docs#chocolate) for more details on chocolate
/// ingredients and the terminology used to define its various constituent components, e.g. _cacao_
/// solids, cocoa butter, _cocoa_ solids, etc.
///
/// The relation of the above components is `cacao solids = cocoa butter + cocoa solids`. The
/// [`sugars`](Self::sugars) content of chocolate ingredients is optional, assumed to be zero if not
/// specified, as some chocolates (e.g. Unsweetened Chocolate, 100% Dark Chocolate) do not contain
/// any added sugars. Any non-zero sugar content is specified in [`Composition`] accessible via
/// [`CompKey::TotalSugars`]. The [`other_solids`](Self::other_solids) content is optional, assumed
/// to be zero if not specified, and represents other non-sugar, non-fats solids, e.g. emulsifiers,
/// impurities in demerara sugar, etc. If non-zero, it is specified in [`Composition`] accessible
/// via [`CompKey::OtherSNFS`]. [`cacao_solids`](Self::cacao_solids), [`sugars`](Self::sugars), and
/// [`other_solids`](Self::other_solids) together must add up to 100%.
///
/// The cocoa solids content is further broken down into proteins, carbohydrates - including fiber,
/// and ash based on standard values for cocoa solids, specified in
/// [`constants::composition::cacao`], e.g. [`cacao::STD_PROTEIN_IN_COCOA_SOLIDS`],
/// [`cacao::STD_CARBOHYDRATES_IN_NATURAL_COCOA_SOLIDS`], [`cacao::STD_FIBER_IN_COCOA_SOLIDS`], etc.
///
/// **Note:** This spec does not model intrinsic sugars in cocoa solids, i.e. those naturally
/// occurring in the cacao nuts - see [`cacao::STD_SUGARS_IN_COCOA_SOLIDS`]. Listings typically only
/// declare total sugars, which include both intrinsic and added. This spec treats all of them as
/// added sugars, stored in [`Solids::other`], accessible via [`CompKey::TotalSugars`].
///
/// # Examples
///
/// (Lindt 70% Cacao Dark Chocolate, 2025)[^107] per 40g serving:
/// - Cacao solids: 70%
/// - Cocoa butter: 16g fat => 40%
/// - Sugars: 12g => 30%
///
/// ```
/// # fn main() -> Result<(), Box<dyn std::error::Error>> {
/// # use sci_cream::docs::assert_eq_float;
/// use sci_cream::{
///     composition::{CompKey, ToComposition},
///     specs::ChocolateSpec
/// };
///
/// let comp = ChocolateSpec {
///     cacao_solids: 70.0,
///     cocoa_butter: Some(40.0),
///     sugars: Some(30.0),
///     other_solids: None,
/// }.to_composition()?;
///
/// assert_eq!(comp.get(CompKey::Sucrose), 30.0);
/// assert_eq!(comp.get(CompKey::CacaoSolids), 70.0);
/// assert_eq!(comp.get(CompKey::CocoaButter), 40.0);
/// assert_eq!(comp.get(CompKey::CocoaSolids), 30.0);
///
/// assert_eq_float!(comp.get(CompKey::Energy), 545.52);
/// assert_eq!(comp.get(CompKey::TotalFats), 40.0);
/// assert_eq!(comp.get(CompKey::TotalProteins), 7.05);
/// assert_eq!(comp.get(CompKey::TotalFiber), 11.34);
/// # Ok(()) }
/// ```
#[doc = include_str!("../../docs/references/index/107.md")]
// @todo Add a `msnf` field to support milk chocolate products (some professional chocolatiers use)
#[derive(PartialEq, Serialize, Deserialize, Copy, Clone, Debug)]
#[serde(deny_unknown_fields)]
pub struct ChocolateSpec {
    /// Total cacao solids content, usually advertised on packaging, e.g. 70% for 70% dark chocolate
    pub cacao_solids: f64,
    /// Cocoa butter content as a percentage of the product as a whole, usually from nutrition facts
    ///
    /// If not specified, it is calculated from [`cacao_solids`](Self::cacao_solids) and the
    /// standard composition [`cacao::STD_COCOA_BUTTER_IN_CACAO_SOLIDS`]. It must be a subset of
    /// [`cacao_solids`](Self::cacao_solids) if both are specified.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub cocoa_butter: Option<f64>,
    /// Sugars content as a percentage of the product as a whole, usually from nutrition facts.
    ///
    /// Assumed to be zero if not specified, as some chocolate products (e.g. Unsweetened Chocolate,
    /// 100% Dark Chocolate) do not contain any added sugars.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub sugars: Option<f64>,
    /// Other solids content as a percentage of the product as a whole, usually from nutrition facts
    ///
    /// Assumed to be zero if not specified, and represents other non-sugar, non-fats solids, e.g.
    /// emulsifiers, impurities in demerara sugar, etc.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub other_solids: Option<f64>,
}

impl ToComposition for ChocolateSpec {
    fn to_composition(&self) -> Result<Composition> {
        let Self {
            cacao_solids,
            cocoa_butter,
            sugars,
            other_solids,
        } = *self;

        let cocoa_butter = cocoa_butter.unwrap_or(cacao_solids * STD_COCOA_BUTTER_IN_CACAO_SOLIDS);
        let sugars = sugars.unwrap_or(0.0);
        let other_solids = other_solids.unwrap_or(0.0);

        verify_are_positive(&[cacao_solids, cocoa_butter, sugars, other_solids])?;
        verify_is_subset(cocoa_butter, cacao_solids, "cocoa_butter <= cacao_solids")?;
        verify_is_100_percent(cacao_solids + sugars + other_solids)?;

        let cocoa_solids = make_cocoa_solids(cacao_solids, cocoa_butter, false)?;
        let sugars = Sugars::new().sucrose(sugars);
        let other_solids = SimpleSolids::new()
            .carbohydrates(Carbohydrates::new().sugars(sugars))
            .others(other_solids);

        Composition::new()
            .energy(cocoa_solids.energy()? + other_solids.energy()?)
            .solids(Solids::new().cocoa(cocoa_solids).other(other_solids))
            .pod(sugars.to_pod()?)
            .pac(
                PAC::new()
                    .sugars(sugars.to_pac()?)
                    .hardness_factor(calculate_cocoa_hardness(cacao_solids, cocoa_butter)),
            )
            .validate_into()
    }
}

/// Spec for cocoa powder ingredients, with cocoa butter and optional other solids
///
/// See the [chocolate documentation](crate::docs#chocolate) for more details on chocolate
/// ingredients and the terminology used to define its various constituent components, e.g. _cacao_
/// solids, cocoa butter, _cocoa_ solids, etc.
///
/// The relation of the above components is `cacao solids = cocoa butter + cocoa solids`. The
/// [`cacao_solids`](Self::cacao_solids) content is optional, calculated as `100 - water -
/// other_solids` if not specified. It must be a superset of [`cocoa_butter`](Self::cocoa_butter),
/// which must be specified, typically ranging from ~10-24%. The
/// [`other_solids`](Self::other_solids) content is optional, assumed to be zero if unspecified, and
/// represents other non-sugar, non-fats solids, e.g. emulsifiers, demerara sugar impurities,
/// alkalization ash, etc. If non-zero, it is specified in [`Composition`] accessible via
/// [`CompKey::OtherSNFS`]. [`cacao_solids`](Self::cacao_solids),
/// [`other_solids`](Self::other_solids), and [`water`](Self::water) must add up to 100%.
///
/// The cocoa solids content is further broken down into proteins, carbohydrates - including fiber,
/// and ash based on standard values for cocoa solids, specified in
/// [`constants::composition::cacao`], e.g. [`cacao::STD_PROTEIN_IN_COCOA_SOLIDS`],
/// [`cacao::STD_CARBOHYDRATES_IN_NATURAL_COCOA_SOLIDS`], [`cacao::STD_FIBER_IN_COCOA_SOLIDS`], etc.
///
/// **Note:** This spec does not model intrinsic sugars in cocoa solids, i.e. those naturally
/// occurring in the cacao nuts - see [`cacao::STD_SUGARS_IN_COCOA_SOLIDS`]. They are included as
/// cacao and cocoa solids, which is correct, but are not tracked as sugars and are not included
/// in [`CompKey::TotalSugars`], nor do they contribute to sweetness or freezing point depression.
///
/// # Examples
///
/// (Ghirardelli 100% Unsweetened Cocoa Powder, 2025)[^111] per 6g serving:
/// - Cocoa butter: 1g fat => 16.67%
/// - Cacao solids: 100% less the default 3% water => 97% (calculated internally)
///
/// ```
/// # fn main() -> Result<(), Box<dyn std::error::Error>> {
/// # use sci_cream::docs::assert_eq_float;
/// # use sci_cream::{
/// #     composition::{CompKey, ToComposition},
/// #     specs::CocoaPowderSpec
/// # };
/// #
/// let comp = CocoaPowderSpec {
///     cacao_solids: None,
///     cocoa_butter: 16.67,
///     other_solids: None,
///     water: None,
///     dutch_processed: None,
/// }.to_composition()?;
///
/// assert_eq!(comp.get(CompKey::TotalSweeteners), 0.0);
/// assert_eq_float!(comp.get(CompKey::Water), 3.0);
/// assert_eq_float!(comp.get(CompKey::CacaoSolids), 97.0);
/// assert_eq!(comp.get(CompKey::CocoaButter), 16.67);
/// assert_eq_float!(comp.get(CompKey::CocoaSolids), 80.33);
///
/// assert_eq_float!(comp.get(CompKey::Energy), 325.47072);
/// assert_eq!(comp.get(CompKey::TotalFats), 16.67);
/// assert_eq_float!(comp.get(CompKey::TotalProteins), 18.87755);
/// assert_eq_float!(comp.get(CompKey::TotalFiber), 30.36474);
/// # Ok(()) }
/// ```
#[doc = include_str!("../../docs/references/index/111.md")]
#[derive(PartialEq, Serialize, Deserialize, Copy, Clone, Debug)]
#[serde(deny_unknown_fields)]
pub struct CocoaPowderSpec {
    /// Total cacao solids content, optional, `100 - water - other_solids` if not specified
    #[serde(skip_serializing_if = "Option::is_none")]
    pub cacao_solids: Option<f64>,
    /// Cocoa butter content as a percentage of the product as a whole, usually from nutrition facts
    ///
    /// It must be specified, and must be a subset of [`cacao_solids`](Self::cacao_solids)
    pub cocoa_butter: f64,
    /// Other solids content as a percentage of the product as a whole, usually from nutrition facts
    ///
    /// Assumed to be zero if not specified, and represents other non-sugar, non-fats solids, e.g.
    /// emulsifiers, impurities in demerara sugar, alkalization ash, etc.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub other_solids: Option<f64>,
    /// Water content as a percentage of the product as a whole
    ///
    /// Defaults to [`cacao::STD_WATER_IN_COCOA_POWDER`] if not specified, which is usually the case
    /// for most commercial products. A ceiling can sometimes be inferred from regulatory standards,
    /// e.g. EU caps water in cocoa powder to 9% (Directive 2000/36/EC, 2000, Annex I 2.(a))[^84].
    #[doc = include_str!("../../docs/references/index/84.md")]
    #[serde(skip_serializing_if = "Option::is_none")]
    pub water: Option<f64>,
    /// Whether the powder is alkalized, also known as dutch processing, `false` if unspecified
    ///
    /// Alkalized cocoa powder has been treated with an alkalizing agent. This affects the
    /// composition of the cocoa solids, namely the distribution of ash and carbohydrates; see
    /// [`cacao::STD_ASH_IN_NATURAL_COCOA_SOLIDS`] and [`cacao::STD_ASH_IN_DUTCHED_COCOA_SOLIDS`].
    /// See the [dutch processed](crate::docs#dutch-processed) documentation for more details.
    ///
    /// **Note:** Dutch processing is a graded scale and there are degrees of alkalization, ranging
    /// from lightly to heavily alkalized cocoa powders (Miller et al., 2008)[^85]. As such, this
    /// binary classification only approximates a typical moderate level of alkalization.
    #[doc = include_str!("../../docs/references/index/85.md")]
    #[serde(skip_serializing_if = "Option::is_none")]
    pub dutch_processed: Option<bool>,
}

impl ToComposition for CocoaPowderSpec {
    fn to_composition(&self) -> Result<Composition> {
        let Self {
            cacao_solids,
            cocoa_butter,
            other_solids,
            water,
            dutch_processed,
        } = *self;

        let other_solids = other_solids.unwrap_or(0.0);
        let water = water.unwrap_or(STD_WATER_IN_COCOA_POWDER * 100.0);
        let cacao_solids = cacao_solids.unwrap_or(100.0 - water - other_solids);

        verify_are_positive(&[cacao_solids, cocoa_butter, other_solids, water])?;
        verify_is_subset(cocoa_butter, cacao_solids, "cocoa_butter <= cacao_solids")?;
        verify_is_100_percent(cacao_solids + other_solids + water)?;

        let cocoa_solids = make_cocoa_solids(cacao_solids, cocoa_butter, dutch_processed.unwrap_or(false))?;
        let other_solids = SimpleSolids::new().others(other_solids);

        Composition::new()
            .energy(cocoa_solids.energy()? + other_solids.energy()?)
            .solids(Solids::new().cocoa(cocoa_solids).other(other_solids))
            .pac(PAC::new().hardness_factor(calculate_cocoa_hardness(cacao_solids, cocoa_butter)))
            .validate_into()
    }
}

/// Creates a [`SimpleSolids`] representing the specified `cacao_solids` and `cocoa_butter`
///
/// Alkalization leaves a mineral residue that raises ash at the expense of carbohydrates, so
/// `dutch_processed` selects between the two pairs of standard fractions.
fn make_cocoa_solids(cacao_solids: f64, cocoa_butter: f64, dutch_processed: bool) -> Result<SimpleSolids> {
    let cocoa_snf = cacao_solids - cocoa_butter;

    let (std_carbohydrates_in_cocoa_solids, std_ash_in_cocoa_solids) = if dutch_processed {
        (STD_CARBOHYDRATES_IN_DUTCHED_COCOA_SOLIDS, STD_ASH_IN_DUTCHED_COCOA_SOLIDS)
    } else {
        (STD_CARBOHYDRATES_IN_NATURAL_COCOA_SOLIDS, STD_ASH_IN_NATURAL_COCOA_SOLIDS)
    };

    SimpleSolids::new()
        .fats(
            Fats::new()
                .total(cocoa_butter)
                .saturated(cocoa_butter * STD_SATURATED_FAT_IN_COCOA_BUTTER),
        )
        .proteins(SimpleProteins::from_total(cocoa_snf * STD_PROTEIN_IN_COCOA_SOLIDS))
        .carbohydrates(
            Carbohydrates::new()
                .fiber(Fibers::new().other(cocoa_snf * STD_FIBER_IN_COCOA_SOLIDS))
                .others_from_total(cocoa_snf * std_carbohydrates_in_cocoa_solids)?,
        )
        .others(cocoa_snf * std_ash_in_cocoa_solids)
        .validate_into()
}

/// Calculates the hardness factor based on the specified `cacao_solids` and `cocoa_butter`
fn calculate_cocoa_hardness(cacao_solids: f64, cocoa_butter: f64) -> f64 {
    let cocoa_snf = cacao_solids - cocoa_butter;
    cocoa_butter * hf::CACAO_BUTTER + cocoa_snf * hf::COCOA_SOLIDS
}

#[cfg(test)]
#[cfg_attr(coverage, coverage(off))]
#[allow(clippy::unwrap_used, clippy::float_cmp)]
pub(crate) mod tests {
    use std::sync::LazyLock;

    use crate::tests::asserts::shadow_asserts::assert_eq;
    use crate::tests::asserts::*;

    use super::*;
    use crate::{
        composition::{CompKey, SolidsBreakdown},
        error::Error,
        ingredient::Category,
        specs::IngredientSpec,
    };

    fn empty_chocolate_spec() -> ChocolateSpec {
        ChocolateSpec {
            cacao_solids: 0.0,
            cocoa_butter: None,
            sugars: None,
            other_solids: None,
        }
    }

    fn empty_cocoa_powder_spec() -> CocoaPowderSpec {
        CocoaPowderSpec {
            cacao_solids: None,
            cocoa_butter: 0.0,
            other_solids: None,
            water: None,
            dutch_processed: None,
        }
    }

    pub(crate) const ING_SPEC_CHOCOLATE_LINDT_70_DARK_CHOCOLATE_STR: &str = r#"{
      "name": "Lindt EXCELLENCE 70% Cacao Dark Chocolate",
      "category": "Chocolate",
      "ChocolateSpec": {
        "cacao_solids": 70,
        "cocoa_butter": 40,
        "sugars": 30
      }
    }"#;

    pub(crate) static ING_SPEC_CHOCOLATE_LINDT_70_DARK_CHOCOLATE: LazyLock<IngredientSpec> =
        LazyLock::new(|| IngredientSpec {
            name: "Lindt EXCELLENCE 70% Cacao Dark Chocolate".to_string(),
            category: Category::Chocolate,
            spec: ChocolateSpec {
                cacao_solids: 70.0,
                cocoa_butter: Some(40.0),
                sugars: Some(30.0),
                ..empty_chocolate_spec()
            }
            .into(),
        });

    pub(crate) static COMP_LINDT_70_DARK_CHOCOLATE: LazyLock<Composition> = LazyLock::new(|| {
        Composition::new()
            .energy(545.52)
            .solids(
                Solids::new()
                    .cocoa(
                        SolidsBreakdown::new()
                            .fats(Fats::new().total(40.0).saturated(24.0))
                            .carbohydrates(
                                Carbohydrates::new()
                                    .fiber(Fibers::new().other(11.34))
                                    .others_from_total(20.67)
                                    .unwrap(),
                            )
                            .proteins(SimpleProteins::from_total(7.05))
                            .others(2.28),
                    )
                    .other(
                        SolidsBreakdown::new().carbohydrates(Carbohydrates::new().sugars(Sugars::new().sucrose(30.0))),
                    ),
            )
            .pod(30.0)
            .pac(PAC::new().sugars(30.0).hardness_factor(90.0))
    });

    #[test]
    fn to_composition_chocolate_spec_lindt_70_dark_chocolate() {
        let comp = ING_SPEC_CHOCOLATE_LINDT_70_DARK_CHOCOLATE
            .spec
            .to_composition()
            .unwrap();

        assert_eq_flt_test!(comp.get(CompKey::Energy), 545.52);
        assert_eq!(comp.get(CompKey::TotalFats), 40.0);
        assert_eq!(comp.get(CompKey::TotalProteins), 7.05);
        assert_eq!(comp.get(CompKey::TotalFiber), 11.34);

        assert_eq!(comp.get(CompKey::Sucrose), 30.0);
        assert_eq!(comp.get(CompKey::TotalSweeteners), 30.0);

        // Added sugars in chocolates is considered part of total sweeteners, not of Cacao Solids
        assert_eq!(comp.get(CompKey::CacaoSolids), comp.get(CompKey::TotalSolids) - comp.get(CompKey::TotalSweeteners));
        assert_eq!(comp.get(CompKey::CacaoSolids), 70.0);
        assert_eq!(comp.get(CompKey::CocoaButter), 40.0);
        assert_eq!(comp.get(CompKey::CocoaSolids), 30.0);
        assert_eq!(comp.solids.cocoa.others, 2.28);
        assert_eq!(comp.get(CompKey::OtherSNFS), 0.0);
        assert_eq_flt_test!(comp.get(CompKey::TotalSNFS), 30.0);
        assert_eq!(comp.get(CompKey::TotalSolids), 100.0);
        assert_eq!(comp.get(CompKey::POD), 30.0);
        assert_eq!(comp.get(CompKey::TotalPAC), 30.0);
        assert_eq!(comp.get(CompKey::HF), 90.0);

        assert_eq!(comp.get(CompKey::SaturatedFat), 24.0);
        assert_eq!(comp.get(CompKey::TransFat), 0.0);
    }

    pub(crate) const ING_SPEC_CHOCOLATE_LINDT_95_DARK_CHOCOLATE_OTHER_SOLIDS_STR: &str = r#"{
      "name": "Lindt EXCELLENCE 95% Cacao Dark Chocolate",
      "category": "Chocolate",
      "ChocolateSpec": {
        "cacao_solids": 95,
        "cocoa_butter": 57.5,
        "sugars": 3,
        "other_solids": 2
      }
    }"#;

    pub(crate) static ING_SPEC_CHOCOLATE_LINDT_95_DARK_CHOCOLATE_OTHER_SOLIDS: LazyLock<IngredientSpec> =
        LazyLock::new(|| IngredientSpec {
            name: "Lindt EXCELLENCE 95% Cacao Dark Chocolate".to_string(),
            category: Category::Chocolate,
            spec: ChocolateSpec {
                cacao_solids: 95.0,
                cocoa_butter: Some(57.5),
                sugars: Some(3.0),
                other_solids: Some(2.0),
            }
            .into(),
        });

    pub(crate) static COMP_LINDT_95_DARK_CHOCOLATE: LazyLock<Composition> = LazyLock::new(|| {
        Composition::new()
            .energy(611.4)
            .solids(
                Solids::new()
                    .cocoa(
                        SolidsBreakdown::new()
                            .fats(Fats::new().total(57.5).saturated(34.5))
                            .carbohydrates(
                                Carbohydrates::new()
                                    .fiber(Fibers::new().other(14.175))
                                    .others_from_total(25.8375)
                                    .unwrap(),
                            )
                            .proteins(SimpleProteins::from_total(8.8125))
                            .others(2.85),
                    )
                    .other(
                        SolidsBreakdown::new()
                            .carbohydrates(Carbohydrates::new().sugars(Sugars::new().sucrose(3.0)))
                            .others(2.0),
                    ),
            )
            .pod(3.0)
            .pac(PAC::new().sugars(3.0).hardness_factor(119.25))
    });

    #[test]
    fn to_composition_chocolate_spec_lindt_95_dark_chocolate_other_solids() {
        let comp = ING_SPEC_CHOCOLATE_LINDT_95_DARK_CHOCOLATE_OTHER_SOLIDS
            .spec
            .to_composition()
            .unwrap();

        assert_eq_flt_test!(comp.get(CompKey::Energy), 611.4);
        assert_eq!(comp.get(CompKey::TotalFats), 57.5);
        assert_eq_flt_test!(comp.get(CompKey::TotalProteins), 8.8125);
        assert_eq_flt_test!(comp.get(CompKey::TotalFiber), 14.175);
        assert_eq!(comp.get(CompKey::TotalSweeteners), 3.0);

        assert_eq!(comp.get(CompKey::CacaoSolids), 95.0);
        assert_eq!(comp.get(CompKey::CocoaButter), 57.5);
        assert_eq!(comp.get(CompKey::CocoaSolids), 37.5);
        assert_eq_flt_test!(comp.solids.cocoa.others, 2.85);
        assert_eq!(comp.solids.other.others, 2.0);
        assert_eq_flt_test!(comp.get(CompKey::TotalCarbohydrates), 28.8375);
        assert_eq!(comp.get(CompKey::OtherSNFS), 2.0);
        assert_eq!(comp.get(CompKey::TotalSNFS), 39.5);
        assert_eq!(comp.get(CompKey::TotalSolids), 100.0);
        assert_eq!(comp.get(CompKey::POD), 3.0);
        assert_eq!(comp.get(CompKey::TotalPAC), 3.0);
        assert_eq!(comp.get(CompKey::HF), 119.25);

        assert_eq!(comp.get(CompKey::SaturatedFat), 34.5);
        assert_eq!(comp.get(CompKey::TransFat), 0.0);
    }

    pub(crate) const ING_SPEC_CHOCOLATE_LINDT_100_DARK_CHOCOLATE_STR: &str = r#"{
      "name": "Lindt EXCELLENCE 100% Cacao Dark Chocolate",
      "category": "Chocolate",
      "ChocolateSpec": {
        "cacao_solids": 100,
        "cocoa_butter": 54
      }
    }"#;

    pub(crate) static ING_SPEC_CHOCOLATE_LINDT_100_DARK_CHOCOLATE: LazyLock<IngredientSpec> =
        LazyLock::new(|| IngredientSpec {
            name: "Lindt EXCELLENCE 100% Cacao Dark Chocolate".to_string(),
            category: Category::Chocolate,
            spec: ChocolateSpec {
                cacao_solids: 100.0,
                cocoa_butter: Some(54.0),
                ..empty_chocolate_spec()
            }
            .into(),
        });

    pub(crate) static COMP_LINDT_100_DARK_CHOCOLATE: LazyLock<Composition> = LazyLock::new(|| {
        Composition::new()
            .energy(586.464)
            .solids(
                Solids::new().cocoa(
                    SolidsBreakdown::new()
                        .fats(Fats::new().total(54.0).saturated(32.4))
                        .carbohydrates(
                            Carbohydrates::new()
                                .fiber(Fibers::new().other(17.388))
                                .others_from_total(31.694)
                                .unwrap(),
                        )
                        .proteins(SimpleProteins::from_total(10.81))
                        .others(3.496),
                ),
            )
            .pod(0.0)
            .pac(PAC::new().hardness_factor(131.4))
    });

    #[test]
    fn to_composition_chocolate_spec_lindt_100_dark_chocolate() {
        let comp = ING_SPEC_CHOCOLATE_LINDT_100_DARK_CHOCOLATE
            .spec
            .to_composition()
            .unwrap();

        assert_eq_flt_test!(comp.get(CompKey::Energy), 586.464);
        assert_eq!(comp.get(CompKey::TotalFats), 54.0);
        assert_eq_flt_test!(comp.get(CompKey::TotalProteins), 10.81);
        assert_eq_flt_test!(comp.get(CompKey::TotalFiber), 17.388);
        assert_eq!(comp.get(CompKey::TotalSweeteners), 0.0);

        assert_eq!(comp.get(CompKey::CacaoSolids), 100.0);
        assert_eq!(comp.get(CompKey::CocoaButter), 54.0);
        assert_eq!(comp.get(CompKey::CocoaSolids), 46.0);
        assert_eq_flt_test!(comp.solids.cocoa.others, 3.496);
        assert_eq!(comp.get(CompKey::OtherSNFS), 0.0);
        assert_eq!(comp.get(CompKey::TotalSolids), 100.0);
        assert_eq!(comp.get(CompKey::POD), 0.0);
        assert_eq!(comp.get(CompKey::TotalPAC), 0.0);
        assert_eq!(comp.get(CompKey::HF), 131.4);

        assert_eq!(comp.get(CompKey::SaturatedFat), 32.4);
        assert_eq!(comp.get(CompKey::TransFat), 0.0);
    }

    pub(crate) const ING_SPEC_COCOA_POWDER_GHIRARDELLI_100_COCOA_POWDER_STR: &str = r#"{
      "name": "Ghirardelli 100% Unsweetened Cocoa Powder",
      "category": "Chocolate",
      "CocoaPowderSpec": {
        "cocoa_butter": 16.67
      }
    }"#;

    pub(crate) static ING_SPEC_COCOA_POWDER_GHIRARDELLI_100_COCOA_POWDER: LazyLock<IngredientSpec> =
        LazyLock::new(|| IngredientSpec {
            name: "Ghirardelli 100% Unsweetened Cocoa Powder".to_string(),
            category: Category::Chocolate,
            spec: CocoaPowderSpec {
                cocoa_butter: 16.67,
                ..empty_cocoa_powder_spec()
            }
            .into(),
        });

    pub(crate) static COMP_GHIRARDELLI_100_COCOA_POWDER: LazyLock<Composition> = LazyLock::new(|| {
        Composition::new()
            .energy(325.47072)
            .solids(
                Solids::new().cocoa(
                    SolidsBreakdown::new()
                        .fats(Fats::new().total(16.67).saturated(10.002))
                        .carbohydrates(
                            Carbohydrates::new()
                                .fiber(Fibers::new().other(30.36474))
                                .others_from_total(55.34737)
                                .unwrap(),
                        )
                        .proteins(SimpleProteins::from_total(18.87755))
                        .others(6.10508),
                ),
            )
            .pod(0.0)
            .pac(PAC::new().hardness_factor(159.597))
    });

    #[test]
    fn to_composition_chocolate_spec_ghirardelli_100_cocoa_powder() {
        let comp = ING_SPEC_COCOA_POWDER_GHIRARDELLI_100_COCOA_POWDER
            .spec
            .to_composition()
            .unwrap();

        // Different similar products list the energy from 250 to 325
        assert_eq_flt_test!(comp.get(CompKey::Energy), 325.47072);
        assert_eq!(comp.get(CompKey::TotalFats), 16.67);
        assert_eq_flt_test!(comp.get(CompKey::TotalProteins), 18.87755);
        assert_eq_flt_test!(comp.get(CompKey::TotalFiber), 30.36474);
        assert_eq!(comp.get(CompKey::TotalSweeteners), 0.0);

        assert_eq_flt_test!(comp.get(CompKey::CacaoSolids), 97.0);
        assert_eq!(comp.get(CompKey::CocoaButter), 16.67);
        assert_eq_flt_test!(comp.get(CompKey::CocoaSolids), 80.33);
        assert_eq_flt_test!(comp.solids.cocoa.others, 6.10508);
        assert_eq!(comp.get(CompKey::OtherSNFS), 0.0);
        assert_eq_flt_test!(comp.get(CompKey::TotalSolids), 97.0);
        assert_eq_flt_test!(comp.get(CompKey::Water), 3.0);
        assert_eq!(comp.get(CompKey::POD), 0.0);
        assert_eq!(comp.get(CompKey::TotalPAC), 0.0);
        assert_eq_flt_test!(comp.get(CompKey::HF), 159.597);

        assert_eq!(comp.get(CompKey::SaturatedFat), 10.002);
        assert_eq!(comp.get(CompKey::TransFat), 0.0);
    }

    pub(crate) const ING_SPEC_COCOA_POWDER_VALRHONA_COCOA_POWDER_STR: &str = r#"{
      "name": "Valrhona Unsweetened Cocoa Powder",
      "category": "Chocolate",
      "CocoaPowderSpec": {
        "cacao_solids": 93,
        "cocoa_butter": 21,
        "other_solids": 4,
        "water": 3,
        "dutch_processed": true
      }
    }"#;

    pub(crate) static ING_SPEC_COCOA_POWDER_VALRHONA_COCOA_POWDER: LazyLock<IngredientSpec> =
        LazyLock::new(|| IngredientSpec {
            name: "Valrhona Unsweetened Cocoa Powder".to_string(),
            category: Category::Chocolate,
            spec: CocoaPowderSpec {
                cacao_solids: Some(93.0),
                cocoa_butter: 21.0,
                other_solids: Some(4.0),
                water: Some(3.0),
                dutch_processed: Some(true),
            }
            .into(),
        });

    pub(crate) static COMP_VALRHONA_COCOA_POWDER: LazyLock<Composition> = LazyLock::new(|| {
        Composition::new()
            .energy(341.352)
            .solids(
                Solids::new()
                    .cocoa(
                        SolidsBreakdown::new()
                            .fats(Fats::new().total(21.0).saturated(12.6))
                            .carbohydrates(
                                Carbohydrates::new()
                                    .fiber(Fibers::new().other(27.216))
                                    .others_from_total(48.384)
                                    .unwrap(),
                            )
                            .proteins(SimpleProteins::from_total(16.92))
                            .others(6.696),
                    )
                    .other(SolidsBreakdown::new().others(4.0)),
            )
            .pod(0.0)
            .pac(PAC::new().hardness_factor(148.5))
    });

    #[test]
    fn to_composition_chocolate_spec_valrhona_cocoa_powder() {
        let comp = ING_SPEC_COCOA_POWDER_VALRHONA_COCOA_POWDER
            .spec
            .to_composition()
            .unwrap();

        assert_eq_flt_test!(comp.get(CompKey::Energy), 341.352);
        assert_eq!(comp.get(CompKey::TotalFats), 21.0);
        assert_eq_flt_test!(comp.get(CompKey::TotalProteins), 16.92);
        assert_eq!(comp.get(CompKey::TotalFiber), 27.216);
        assert_eq!(comp.get(CompKey::TotalSweeteners), 0.0);

        // The declared 93% cacao solids leaves 7% unaccounted for, modeled as `other_solids`
        assert_eq_flt_test!(comp.get(CompKey::CacaoSolids), 93.0);
        assert_eq!(comp.get(CompKey::CocoaButter), 21.0);
        assert_eq_flt_test!(comp.get(CompKey::CocoaSolids), 72.0);
        assert_eq_flt_test!(comp.solids.cocoa.others, 6.696);
        assert_eq!(comp.solids.other.others, 4.0);
        assert_eq_flt_test!(comp.get(CompKey::TotalCarbohydrates), 48.384);
        assert_eq!(comp.get(CompKey::OtherSNFS), 4.0);
        assert_eq_flt_test!(comp.get(CompKey::TotalSNFS), 76.0);
        assert_eq_flt_test!(comp.get(CompKey::TotalSolids), 97.0);
        assert_eq_flt_test!(comp.get(CompKey::Water), 3.0);
        assert_eq!(comp.get(CompKey::POD), 0.0);
        assert_eq!(comp.get(CompKey::TotalPAC), 0.0);
        assert_eq!(comp.get(CompKey::HF), 148.5);

        assert_eq!(comp.get(CompKey::SaturatedFat), 12.6);
        assert_eq!(comp.get(CompKey::TransFat), 0.0);
    }

    /// Omitting cacao solids derives `100 - water - other_solids`, matching Valrhona's declared 93
    #[test]
    fn to_composition_cocoa_powder_spec_derives_cacao_solids_from_other_solids() {
        let comp = CocoaPowderSpec {
            cacao_solids: None,
            cocoa_butter: 21.0,
            other_solids: Some(4.0),
            water: None,
            dutch_processed: Some(true),
        }
        .to_composition()
        .unwrap();

        assert_eq_flt_test!(&comp, &*COMP_VALRHONA_COCOA_POWDER);
    }

    /// An explicit water content is honoured in place of [`cacao::STD_WATER_IN_COCOA_POWDER`]
    #[test]
    fn to_composition_cocoa_powder_spec_uses_explicit_water() {
        // The EU ceiling and a fully dried powder, both far from the 3% default
        for (water, exp_cacao_solids) in [(9.0, 91.0), (0.0, 100.0)] {
            let comp = CocoaPowderSpec {
                cocoa_butter: 10.0,
                water: Some(water),
                ..empty_cocoa_powder_spec()
            }
            .to_composition()
            .unwrap();

            assert_eq_flt_test!(comp.get(CompKey::Water), water);
            assert_eq_flt_test!(comp.get(CompKey::CacaoSolids), exp_cacao_solids);
            assert_eq_flt_test!(comp.get(CompKey::TotalSolids), exp_cacao_solids);
            assert_eq_flt_test!(comp.get(CompKey::CocoaSolids), exp_cacao_solids - 10.0);
        }
    }

    pub(crate) const ING_SPEC_CHOCOLATE_70_DARK_CHOCOLATE_STR: &str = r#"{
      "name": "70% Dark Chocolate",
      "category": "Chocolate",
      "ChocolateSpec": {
        "cacao_solids": 70,
        "sugars": 30
      }
    }"#;

    pub(crate) static ING_SPEC_CHOCOLATE_70_DARK_CHOCOLATE: LazyLock<IngredientSpec> =
        LazyLock::new(|| IngredientSpec {
            name: "70% Dark Chocolate".to_string(),
            category: Category::Chocolate,
            spec: ChocolateSpec {
                cacao_solids: 70.0,
                cocoa_butter: None,
                sugars: Some(30.0),
                ..empty_chocolate_spec()
            }
            .into(),
        });

    pub(crate) static COMP_70_DARK_CHOCOLATE: LazyLock<Composition> = LazyLock::new(|| {
        Composition::new()
            .energy(544.8384)
            .solids(
                Solids::new()
                    .cocoa(
                        SolidsBreakdown::new()
                            .fats(Fats::new().total(39.9).saturated(23.94))
                            .carbohydrates(
                                Carbohydrates::new()
                                    .fiber(Fibers::new().other(11.3778))
                                    .others_from_total(20.7389)
                                    .unwrap(),
                            )
                            .proteins(SimpleProteins::from_total(7.0735))
                            .others(2.2876),
                    )
                    .other(
                        SolidsBreakdown::new().carbohydrates(Carbohydrates::new().sugars(Sugars::new().sucrose(30.0))),
                    ),
            )
            .pod(30.0)
            .pac(PAC::new().sugars(30.0).hardness_factor(90.09))
    });

    #[test]
    fn to_composition_chocolate_spec_70_dark_chocolate() {
        let comp = ING_SPEC_CHOCOLATE_70_DARK_CHOCOLATE.spec.to_composition().unwrap();

        assert_eq_flt_test!(comp.get(CompKey::Energy), 544.8384);
        assert_eq_flt_test!(comp.get(CompKey::TotalFats), 39.9);
        assert_eq_flt_test!(comp.get(CompKey::TotalProteins), 7.0735);
        assert_eq_flt_test!(comp.get(CompKey::TotalFiber), 11.3778);

        assert_eq!(comp.get(CompKey::Sucrose), 30.0);
        assert_eq!(comp.get(CompKey::TotalSweeteners), 30.0);

        assert_eq_flt_test!(comp.get(CompKey::CacaoSolids), 70.0);
        assert_eq_flt_test!(comp.get(CompKey::CocoaButter), 39.9);
        assert_eq_flt_test!(comp.get(CompKey::CocoaSolids), 30.1);
        assert_eq_flt_test!(comp.solids.cocoa.others, 2.2876);
        assert_eq!(comp.get(CompKey::OtherSNFS), 0.0);
        assert_eq_flt_test!(comp.get(CompKey::TotalSNFS), 30.1);
        assert_eq_flt_test!(comp.get(CompKey::TotalSolids), 100.0);
        assert_eq!(comp.get(CompKey::POD), 30.0);
        assert_eq!(comp.get(CompKey::TotalPAC), 30.0);
        assert_eq_flt_test!(comp.get(CompKey::HF), 90.09);

        assert_eq_flt_test!(comp.get(CompKey::SaturatedFat), 23.94);
        assert_eq!(comp.get(CompKey::TransFat), 0.0);
    }

    pub(crate) const ING_SPEC_CHOCOLATE_95_DARK_CHOCOLATE_STR: &str = r#"{
      "name": "95% Dark Chocolate",
      "category": "Chocolate",
      "ChocolateSpec": {
        "cacao_solids": 95,
        "sugars": 5
      }
    }"#;

    pub(crate) static ING_SPEC_CHOCOLATE_95_DARK_CHOCOLATE: LazyLock<IngredientSpec> =
        LazyLock::new(|| IngredientSpec {
            name: "95% Dark Chocolate".to_string(),
            category: Category::Chocolate,
            spec: ChocolateSpec {
                cacao_solids: 95.0,
                cocoa_butter: None,
                sugars: Some(5.0),
                ..empty_chocolate_spec()
            }
            .into(),
        });

    pub(crate) static COMP_95_DARK_CHOCOLATE: LazyLock<Composition> = LazyLock::new(|| {
        Composition::new()
            .energy(596.5664)
            .solids(
                Solids::new()
                    .cocoa(
                        SolidsBreakdown::new()
                            .fats(Fats::new().total(54.15).saturated(32.49))
                            .carbohydrates(
                                Carbohydrates::new()
                                    .fiber(Fibers::new().other(15.4413))
                                    .others_from_total(28.14565)
                                    .unwrap(),
                            )
                            .proteins(SimpleProteins::from_total(9.59975))
                            .others(3.1046),
                    )
                    .other(
                        SolidsBreakdown::new().carbohydrates(Carbohydrates::new().sugars(Sugars::new().sucrose(5.0))),
                    ),
            )
            .pod(5.0)
            .pac(PAC::new().sugars(5.0).hardness_factor(122.265))
    });

    #[test]
    fn to_composition_chocolate_spec_95_dark_chocolate() {
        let comp = ING_SPEC_CHOCOLATE_95_DARK_CHOCOLATE.spec.to_composition().unwrap();

        assert_eq_flt_test!(comp.get(CompKey::Energy), 596.5664);
        assert_eq_flt_test!(comp.get(CompKey::TotalFats), 54.15);
        assert_eq_flt_test!(comp.get(CompKey::TotalProteins), 9.59975);
        assert_eq_flt_test!(comp.get(CompKey::TotalFiber), 15.4413);

        assert_eq!(comp.get(CompKey::Sucrose), 5.0);
        assert_eq!(comp.get(CompKey::TotalSweeteners), 5.0);

        assert_eq_flt_test!(comp.get(CompKey::CacaoSolids), 95.0);
        assert_eq_flt_test!(comp.get(CompKey::CocoaButter), 54.15);
        assert_eq_flt_test!(comp.get(CompKey::CocoaSolids), 40.85);
        assert_eq_flt_test!(comp.solids.cocoa.others, 3.1046);
        assert_eq!(comp.get(CompKey::OtherSNFS), 0.0);
        assert_eq_flt_test!(comp.get(CompKey::TotalSNFS), 40.85);
        assert_eq_flt_test!(comp.get(CompKey::TotalSolids), 100.0);
        assert_eq!(comp.get(CompKey::POD), 5.0);
        assert_eq!(comp.get(CompKey::TotalPAC), 5.0);
        assert_eq_flt_test!(comp.get(CompKey::HF), 122.265);

        assert_eq_flt_test!(comp.get(CompKey::SaturatedFat), 32.49);
        assert_eq!(comp.get(CompKey::TransFat), 0.0);
    }

    pub(crate) const ING_SPEC_CHOCOLATE_100_DARK_CHOCOLATE_STR: &str = r#"{
      "name": "100% Dark Chocolate",
      "category": "Chocolate",
      "ChocolateSpec": {
        "cacao_solids": 100
      }
    }"#;

    pub(crate) static ING_SPEC_CHOCOLATE_100_DARK_CHOCOLATE: LazyLock<IngredientSpec> =
        LazyLock::new(|| IngredientSpec {
            name: "100% Dark Chocolate".to_string(),
            category: Category::Chocolate,
            spec: ChocolateSpec {
                cacao_solids: 100.0,
                cocoa_butter: None,
                sugars: None,
                ..empty_chocolate_spec()
            }
            .into(),
        });

    pub(crate) static COMP_100_DARK_CHOCOLATE: LazyLock<Composition> = LazyLock::new(|| {
        Composition::new()
            .energy(606.912)
            .solids(
                Solids::new().cocoa(
                    SolidsBreakdown::new()
                        .fats(Fats::new().total(57.0).saturated(34.2))
                        .carbohydrates(
                            Carbohydrates::new()
                                .fiber(Fibers::new().other(16.254))
                                .others_from_total(29.627)
                                .unwrap(),
                        )
                        .proteins(SimpleProteins::from_total(10.105))
                        .others(3.268),
                ),
            )
            .pod(0.0)
            .pac(PAC::new().hardness_factor(128.7))
    });

    #[test]
    fn to_composition_chocolate_spec_100_dark_chocolate() {
        let comp = ING_SPEC_CHOCOLATE_100_DARK_CHOCOLATE.spec.to_composition().unwrap();

        assert_eq_flt_test!(comp.get(CompKey::Energy), 606.912);
        assert_eq_flt_test!(comp.get(CompKey::TotalFats), 57.0);
        assert_eq_flt_test!(comp.get(CompKey::TotalProteins), 10.105);
        assert_eq_flt_test!(comp.get(CompKey::TotalFiber), 16.254);
        assert_eq!(comp.get(CompKey::TotalSweeteners), 0.0);

        assert_eq_flt_test!(comp.get(CompKey::CacaoSolids), 100.0);
        assert_eq_flt_test!(comp.get(CompKey::CocoaButter), 57.0);
        assert_eq_flt_test!(comp.get(CompKey::CocoaSolids), 43.0);
        assert_eq_flt_test!(comp.solids.cocoa.others, 3.268);
        assert_eq!(comp.get(CompKey::OtherSNFS), 0.0);
        assert_eq_flt_test!(comp.get(CompKey::TotalSolids), 100.0);
        assert_eq!(comp.get(CompKey::POD), 0.0);
        assert_eq!(comp.get(CompKey::TotalPAC), 0.0);
        assert_eq_flt_test!(comp.get(CompKey::HF), 128.7);

        assert_eq_flt_test!(comp.get(CompKey::SaturatedFat), 34.2);
        assert_eq!(comp.get(CompKey::TransFat), 0.0);
    }

    pub(crate) static INGREDIENT_ASSETS_TABLE_CHOCOLATE: LazyLock<Vec<(&str, IngredientSpec, Option<Composition>)>> =
        LazyLock::new(|| {
            vec![
                (
                    ING_SPEC_CHOCOLATE_LINDT_70_DARK_CHOCOLATE_STR,
                    ING_SPEC_CHOCOLATE_LINDT_70_DARK_CHOCOLATE.clone(),
                    Some(*COMP_LINDT_70_DARK_CHOCOLATE),
                ),
                (
                    ING_SPEC_CHOCOLATE_LINDT_95_DARK_CHOCOLATE_OTHER_SOLIDS_STR,
                    ING_SPEC_CHOCOLATE_LINDT_95_DARK_CHOCOLATE_OTHER_SOLIDS.clone(),
                    Some(*COMP_LINDT_95_DARK_CHOCOLATE),
                ),
                (
                    ING_SPEC_CHOCOLATE_LINDT_100_DARK_CHOCOLATE_STR,
                    ING_SPEC_CHOCOLATE_LINDT_100_DARK_CHOCOLATE.clone(),
                    Some(*COMP_LINDT_100_DARK_CHOCOLATE),
                ),
                (
                    ING_SPEC_COCOA_POWDER_GHIRARDELLI_100_COCOA_POWDER_STR,
                    ING_SPEC_COCOA_POWDER_GHIRARDELLI_100_COCOA_POWDER.clone(),
                    Some(*COMP_GHIRARDELLI_100_COCOA_POWDER),
                ),
                (
                    ING_SPEC_COCOA_POWDER_VALRHONA_COCOA_POWDER_STR,
                    ING_SPEC_COCOA_POWDER_VALRHONA_COCOA_POWDER.clone(),
                    Some(*COMP_VALRHONA_COCOA_POWDER),
                ),
                (
                    ING_SPEC_CHOCOLATE_70_DARK_CHOCOLATE_STR,
                    ING_SPEC_CHOCOLATE_70_DARK_CHOCOLATE.clone(),
                    Some(*COMP_70_DARK_CHOCOLATE),
                ),
                (
                    ING_SPEC_CHOCOLATE_95_DARK_CHOCOLATE_STR,
                    ING_SPEC_CHOCOLATE_95_DARK_CHOCOLATE.clone(),
                    Some(*COMP_95_DARK_CHOCOLATE),
                ),
                (
                    ING_SPEC_CHOCOLATE_100_DARK_CHOCOLATE_STR,
                    ING_SPEC_CHOCOLATE_100_DARK_CHOCOLATE.clone(),
                    Some(*COMP_100_DARK_CHOCOLATE),
                ),
            ]
        });

    #[test]
    fn to_composition_err_on_negative_field() {
        let neg_specs = [
            ChocolateSpec {
                cacao_solids: -1.0,
                cocoa_butter: Some(0.0),
                ..empty_chocolate_spec()
            },
            ChocolateSpec {
                cacao_solids: 70.0,
                cocoa_butter: Some(-1.0),
                ..empty_chocolate_spec()
            },
            ChocolateSpec {
                cacao_solids: 70.0,
                cocoa_butter: Some(40.0),
                sugars: Some(-1.0),
                ..empty_chocolate_spec()
            },
            ChocolateSpec {
                cacao_solids: 70.0,
                cocoa_butter: Some(40.0),
                other_solids: Some(-1.0),
                ..empty_chocolate_spec()
            },
        ];

        for spec in neg_specs {
            let result = spec.to_composition();
            assert!(matches!(result, Err(Error::CompositionNotPositive(_))));
        }
    }

    #[test]
    fn to_composition_err_when_cocoa_butter_exceeds_cacao_solids() {
        let result = ChocolateSpec {
            cacao_solids: 40.0,
            cocoa_butter: Some(60.0),
            ..empty_chocolate_spec()
        }
        .to_composition();
        assert!(matches!(result, Err(Error::InvalidComposition(_))));
    }

    #[test]
    fn to_composition_err_when_components_do_not_sum_to_100() {
        let gt_100_specs = [
            ChocolateSpec {
                cacao_solids: 50.0,
                cocoa_butter: Some(20.0),
                ..empty_chocolate_spec()
            },
            ChocolateSpec {
                cacao_solids: 70.0,
                cocoa_butter: Some(40.0),
                sugars: Some(35.0),
                ..empty_chocolate_spec()
            },
            ChocolateSpec {
                cacao_solids: 70.0,
                cocoa_butter: Some(40.0),
                other_solids: Some(35.0),
                ..empty_chocolate_spec()
            },
        ];

        for spec in gt_100_specs {
            let result = spec.to_composition();
            assert!(matches!(result, Err(Error::CompositionNot100Percent(_))));
        }
    }

    #[test]
    fn to_composition_cocoa_powder_err_when_components_do_not_sum_to_100() {
        let bad_specs = [
            // A full 100% of cacao solids leaves no room for the default water
            CocoaPowderSpec {
                cacao_solids: Some(100.0),
                cocoa_butter: 16.67,
                ..empty_cocoa_powder_spec()
            },
            CocoaPowderSpec {
                cacao_solids: Some(93.0),
                cocoa_butter: 21.0,
                water: Some(10.0),
                ..empty_cocoa_powder_spec()
            },
            CocoaPowderSpec {
                cacao_solids: Some(90.0),
                cocoa_butter: 21.0,
                ..empty_cocoa_powder_spec()
            },
        ];

        for spec in bad_specs {
            let result = spec.to_composition();
            assert!(matches!(result, Err(Error::CompositionNot100Percent(_))));
        }
    }

    #[test]
    fn to_composition_cocoa_powder_err_on_negative_water() {
        // Sums to 100, so the sign check is the only one that can fire
        let result = CocoaPowderSpec {
            cacao_solids: Some(103.0),
            cocoa_butter: 21.0,
            water: Some(-3.0),
            ..empty_cocoa_powder_spec()
        }
        .to_composition();
        assert!(matches!(result, Err(Error::CompositionNotPositive(_))));
    }
}
