...
[freezing point depression](crate::docs#freezing-point-depression), hardness at different
...
curves](crate::docs#freezing-point-depression-curve) of the mix. This can be accessed like this:
...
Dolcificante (POD)](crate::docs#pod), [PAC](crate::docs#pac-afp-fpdf-se), energy in kcal, etc., and
is the basis for all further calculation and analyses, e.g. [Freezing Point Depression
(FPD)](crate::docs#freezing-point-depression) calculations. It is the core of the functionality of
this library.
...
utilizing various sub-structs, their calculations methods, e.g. [`to_pod`](Carbohydrates::to_pod),
[`to_pac`](Carbohydrates::to_pac), [`energy`](SolidsBreakdown::energy), etc. and several constants
...
has several more fields, including [`sweeteners`](SweetenerSpec::sweeteners) holding a
...
reference in order to access their [`Composition`]. This is done via the [`ResolveComposition`] and [`ResolveIntoIngredient`] traits which take an [`IngredientGetter`] that allows them to look up spec
...
# use sci_cream::docs::assert_eq_float;
# use sci_cream::{
#     composition::{CompKey::*, ResolveComposition},
...
#   IngredientDatabase, Recipe, BalanceKey, Priority::{self, *}};
#
# let db = IngredientDatabase::new_seeded_from_embedded_data();
...
#[cfg(feature = "database")]
pub use database::IngredientDatabase;
