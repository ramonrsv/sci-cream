...
pub(crate) mod tests {
    ...
    const EXPECTED_EMBEDDED_SPEC_COUNT: usize = 143;
    const EXPECTED_EMBEDDED_SPEC_NON_ALIAS_COUNT: usize = 129;
    const EXPECTED_EMBEDDED_SPEC_INDEPENDENT_COUNT: usize = 125;
    ...
    fn get_independent_ingredient_spec_by_name_composite() {
        let result = super::get_independent_ingredient_spec_by_name("Underbelly General Purpose Stabilizer Blend");
        assert!(matches!(result, Err(Error::UnsupportedSpec(_))));
    }
